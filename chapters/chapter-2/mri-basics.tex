...
\begin{align}
	\Delta K = & \dfrac{1}{\rltext{FOV}} \\
	 k\rltext{FOV} = & (+k_{\max}) - (-k_{\max}) = 2 k_{\max} \\
	 = & N / \lrtext{FOV} = 1 / \Delta w
\end{align}

\FloatBarrier
\subsection{ارتباط \kspace با کد کردن فرکانس و فاز}
...
\begin{equation}\label{eq:element-fe}
	\mathbf{\partial} s(t) = \rho(x) . \exp(\frac{-t}{T_2^*}) .  \exp(i\gamma.x.G_{\lrtext{FE}}.t). \mathbf{d}x
\end{equation}
